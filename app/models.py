...
class User(db.Model):
    ...
    addtime = db.Column(db.DateTime, default=datetime.now) # 注册日期
...
class Auth(db.Model):
    ...
    url = db.Column(db.String(255), unique=True) # 地址
    addtime = db.Column(db.DateTime, default=datetime.now) #  添加日期
...
class Role(db.Model):
    ...
    addtime = db.Column(db.DateTime, default=datetime.now) # 角色入库添加时间
